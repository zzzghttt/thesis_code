...
class GAT(torch.nn.Module):
    def __init__(self, input_dim, hid_dim=None, out_dim=None, num_layer=3, JK="last", drop_ratio=0.0, pool='mean', heads=4, attn_drop=0.1):
        super().__init__()
        """
        Args:
        ...
        if num_layer < 2:
            ...
            self.conv_layers.append(GraphConv(input_dim, hid_dim, heads=heads, dropout=attn_drop))
            self.norm_layers.append(nn.LayerNorm(hid_dim*heads))

            for i in range(1, num_layer - 1):
                self.conv_layers.append(GraphConv(hid_dim*heads, hid_dim, heads=heads, dropout=attn_drop))
                self.norm_layers.append(nn.LayerNorm(hid_dim*heads))

            self.conv_layers.append(GraphConv(hid_dim*heads, out_dim, heads=1, concat=False, dropout=attn_drop))
