...
def analyze_records(history_path):
        ...
        for file in files:
            if file == "records.json":
                ...
                for round_dict in record_dict:     
                    ...
                    if round_dict.get("hasCode") == False:
                    ...
                    elif "help the user fix the unit test file with error" in round_dict.get("prompt")[0]["content"]:
                        if round_dict.get('round') == 0:
                            generation_error_count += 1
                        else:
                            repair_error_count += 1
                        ...
                        if has_mock_error(round_dict.get("prompt")[0]["content"]):
                            mock_error_count += 1
    ...
    print(f"total error rate: {(result_dict['extract_error_count'] + result_dict['compile_error_count'] + result_dict['runtime_error_count'] + result_dict['unknown_error_count']) / result_dict['request_count'] * 100:.2f}%")
